...
function sortPhrase(phrase){
     //must add .trim() to trim away invisible spaces at the end of array 
...
function isAnagramInArray(anagram, arr){
    ...
    return arr.filter(item => {
        //make use of temporary variable at runtime for comparison
        //allows us to preserve the original shape/state of the object (string in certain order) in comparison while performing comparison in a different one (sorted in alphabetical order)
        //make helper functions to perform one specific task, such as manipulating input to be in certain state for comparsion such as ascending, descending, alphabetical roders.
